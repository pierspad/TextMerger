...
cd build-publish-scripts/linux
...
tar --exclude='.git' \
    ...
    -czf "build-publish-scripts/linux/textmerger-$VERSION.tar.gz" \
    --transform="s,^,TextMerger-$VERSION/," \
    --exclude='build-publish-scripts/linux/src' \
    --exclude='build-publish-scripts/linux/pkg' \
    --exclude='build-publish-scripts/linux/textmerger' \
    .
